...
// An ISO timestamp -> its calendar date in Africa/Johannesburg, e.g. "2026-08-18".
// Use this (not toISOString().slice(0,10), which is UTC) anywhere events are
// bucketed/matched by day - UTC+2 means the UTC and SAST dates disagree for
// events starting between 00:00 and 02:00 SAST.
export function toDateKey(iso) {
  return new Date(iso).toLocaleDateString('en-CA', { timeZone: TZ })
}

// A SAST calendar date ("2026-08-18") -> the [start, end) UTC-equivalent ISO
// bounds of that day, for server-side range queries (e.g. "events on this day").
export function dayRangeUtc(dateKey) {
  const [y, m, d] = dateKey.split('-').map(Number)
  const start = `${dateKey}T00:00:00+02:00`
  const nextDay = new Date(Date.UTC(y, m - 1, d + 1))
  const end = `${nextDay.toISOString().slice(0, 10)}T00:00:00+02:00`
  return { start, end }
}
...
export function splitISO(iso) {
  ...
  const time = new Date(iso).toLocaleTimeString('en-ZA', {
    timeZone: TZ, hour: '2-digit', minute: '2-digit', hour12: false,
  })
  return { date: toDateKey(iso), time }
}
