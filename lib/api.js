...
import { isPast, dayRangeUtc } from './format'
...
// Wraps an ilike pattern in double quotes (PostgREST's escape for values containing
// the `,`/`()` chars its filter-string syntax otherwise treats as separators).
...
export async function fetchUpcomingEvents({
  ...
  let q = supabase.from('upcoming_events').select('*').eq('published', true)

  if (category !== 'all') q = q.eq('category', category)
  if (query.trim()) {
    const pattern = ilikePattern(query.trim())
    q = q.or(`title.ilike.${pattern},organizer_name.ilike.${pattern}`)
  }
  if (dateKey) {
...
// Called by a rejected organiser themselves (not the admin) once they've
// seen the rejection popup - fully wipes their account (organizers row,
// profiles row, and the auth.users login itself) via the reject-cleanup Edge
// Function, which holds the service-role key this client never does.
export async function deleteRejectedOrganizerAccount() {
  const { error } = await supabase.functions.invoke('reject-cleanup')
  if (error) throw error
}
