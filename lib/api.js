...
// seen the rejection popup - deletes their own organizers row and resets
// their profile role back to 'user' via a security definer RPC. See
// leave_rejected_organizer() in supabase_edit_delete_functions.sql.
export async function leaveRejectedOrganizer() {
  const { error } = await supabase.rpc('leave_rejected_organizer')
  if (error) throw error
}
